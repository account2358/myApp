...
def point_inside_csp(p,csp, on_the_path = True) :
	...
	print_(("273 ray_intersections_count=",ray_intersections_count));	
	for subpath in csp :
	 					...
	 					if t == 1 :
							...
							for j in range(1, len(subpath)+1):
								...
								if st_d*en_d <=0 : 
									...
									print_(("313 ray_intersections_count=",ray_intersections_count));	
			 						break 
	 				else :	
		 				x1,y1 = csp_at_t(sp1,sp2,t)
		 				print_("318 x1,y1 = %s,%s"%(x1,y1));
		 				print_("318 y = %s"%(y));
		 				if y1==y : 
		 					# the point is on the path 
		 					print_("317");
		 					...
		 					if y1>y and 3*ax*t**2 + 2*bx*t + cx !=0 : # if it's 0 the path only touches the ray
		 						ray_intersections_count += 1 
		 						print_("324 3*ax*t**2 + 2*bx*t + cx  = 3*%f*%f**2 + 2*%f*%f + %f = %f"%(ax,t,bx,t,cx,3*ax*t**2 + 2*bx*t + cx));
								print_(("324 ray_intersections_count=",ray_intersections_count));	
	print_("322");	
...
#					poly.draw(color = "Violet",width=4)					
	...
	def __init__(self):
		...
		self.OptionParser.add_option("",   "--original-point",				action="store", type="string", 		dest="original_point", default="Left Up",		help="Original Point")
...
################################################################################
	...
	def transform(self,source_point, layer, reverse=False):
		...
		print_(("y=",y));
		return [x,y]
		doc_height = doc_height * t[0][0];
		doc_width = doc_width * t[0][0];
		if self.options.original_point == "Left Down" :
			x = x;
		if self.options.original_point == "Left Up" :
			y = y - doc_height;
		if self.options.original_point == "Right Down" :
			x = x - doc_width;
		if self.options.original_point == "Right Up" :
			y =  y - doc_height;
			x =  x - doc_width;
		if self.options.original_point == "Center" :
			y = y - doc_height/2;
			x = x - doc_width/2;
		return [x,y]
		##return [t[0][0]*x, t[0][0]*y]
		...
		for layer in self.layers :
			if layer in self.selected_paths :
				...
				for path in self.selected_paths[layer]:
					...
					print_(("5096 lines=",lines))
					# get the intersection points
					
					splitted_line = [ [lines[0][0]] ]
					print_(("5100 splitted_line=",splitted_line))
					intersections = {}
					print_(("5102 intersections=",intersections))
					for l1,l2, in zip(lines[0],lines[0][1:]): 
								...
								for t in roots :
									...
									if 0<=t1<=1	:
										...
										if p in intersections :
											...
											print_(("5121 intersections=",intersections))  
										else : 	
											intersections[p]  = [ [i,j,t] ]  
											print_(("5124 intersections[p]=",intersections[p]))
						...
						for i in ints:
							...
							while index_tmp < len(splitted_line):
								print_(("5129 splitted_line[%d]="%index_tmp,splitted_line[index_tmp])) 
								index_tmp += 1;
						...
						index_tmp = 0;
						while index_tmp < len(splitted_line):
							print_(("5129 1 splitted_line[%d]="%index_tmp,splitted_line[index_tmp])) 
							index_tmp += 1;
							
						splitted_line[-1] += [  l2  ]
						i = 0				
					print_(("5130 splitted_line=",splitted_line))
					while i < len(splitted_line) :
						...
						if 3 < len(splitted_line[i]) :
							print_(("5131 "))	
							print_(("5131 splitted_line[%d]="%i,splitted_line[i]))	
							print_(("5131 "))					
							#del splitted_line[i][2]				
							#del splitted_line[i][3]
						...
						index_tmp = 0;
						while index_tmp < len(splitted_line):
							print_(("5141 1 splitted_line[%s]="%index_tmp,splitted_line[index_tmp])) 
							index_tmp += 1;
						print_(("5141 splitted_line[%s][0]="%i,splitted_line[i][0]))
						print_(("5142 splitted_line[%s][1]="%i,splitted_line[i][1]))
						p = [(l1[0]+l2[0])/2, (l1[1]+l2[1])/2]
						print_(("5144 p=",p))
						print_(("5145 csp=",csp))
						if not point_inside_csp(p, csp):
							#i +=1 	
							print_(("5146  del splitted_line[%s]="%i,splitted_line[i]))					
							del splitted_line[i]
						else :
							i += 1
							print_("5150")
